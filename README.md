...
>
> The single positive result reported here (frozen DermLIP at test AUROC 0.944 ± 0.003 on the held-out third nuisance family, EXP-007) **cannot be attributed to dermoscopy-domain transfer** with the experiments in this nine-run sequence. DermLIP's pretraining corpus, [Derm1M](https://arxiv.org/abs/2503.14911), does **not** name HAM10000 among its sources (its named public datasets are SCIN and MSKCC, an ISIC-archive subset), but it documents no deduplication or decontamination against HAM10000, bulk-aggregates PubMed/web/social dermatology content where HAM10000 images circulate freely, and is evaluated on HAM10000 by its own authors (who include the HAM10000 creators) without an overlap caveat. HAM10000 image-level overlap therefore **cannot be excluded**, and a direct overlap audit is gated behind Derm1M access. The BiomedCLIP partition (EXP-008, +0.04 over web CLIP) only rules out the broader "any general-medical pretraining is sufficient" alternative; it does **not** separate dermoscopy-domain transfer from possible HAM10000 image-level overlap.
>
