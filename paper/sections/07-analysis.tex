...
rotation does not partially align --- it anti-aligns. Pair-score
histograms on the test split (mirrored at \texttt{<run\_id>/figures/}
under \runarchive\ for every run) show this directly: stable pairs
receive higher predicted-target cosine distance than changing pairs,
and the inversion is large enough that $\auroc$ sits in the
