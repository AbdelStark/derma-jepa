...
\mathbb{R}^{d}$ minimises the L2-MSE objective on stable training pairs, with
a soft \texttt{weight $-$ I} regulariser holding the function near the
...
\begin{equation}
  ...
  \;+\; \lambda \, \lVert W - I \rVert_{F}^{2},
  \label{eq:loss}
\end{equation}
where $\lambda$ is the configured weight decay and $W$ is the linear weight
(or, for the MLP, the residual weight). At evaluation time, the score for a
