...
saw HAM10000 (or near-duplicate) images during pretraining --- via Derm1M's
uncontrolled literature/web sources, since HAM10000 is not a named Derm1M
source --- and the embedding space is well-fit to HAM10000 lesion identity in
a way that survives our synthetic nuisance. EXP-008's BiomedCLIP partition rules out only the broader ``any
...
NZ, BCN20000 \citep{combalia2019bcn20000} non-HAM10000 components, and
similar publicly available dermoscopy and clinical-skin sources
(any DermQuest content would have to be sourced from a surviving
archival mirror; the original portal has been retired since $2019$). Run a short
...
$[0.80, 0.85)$ & Borderline transfer; near miss. & Headline survives weakened: ``dermoscopy-domain pretraining is a substantial but not full driver,'' with explicit acknowledgement of the borderline. \\
...
$< 0.30$       & Below-random inversion (same regime as EXP-004 / EXP-006a / EXP-006b). & Claim narrows further: ``dermoscopy-domain pretraining without the eval dataset behaves like natural-image pretraining,'' and EXP-007 is then attributable specifically to HAM10000 image-level overlap rather than to dermoscopy pretraining of any kind. \\
\bottomrule
\end{tabular}
\caption{EXP-009 outcome bands on \strongholdtwo\ partitioning $[0,1]$
exhaustively and the corresponding revision of the paper's claim.
Committing this mapping up front prevents post-hoc re-interpretation of
the experiment, including in the borderline and below-random regions.}
