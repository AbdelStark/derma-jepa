...
EXP-002 (and the only one among the six primary-tier runs that
evaluate on \strongholdtwo). The
contamination caveat is introduced here: Derm1M does not name HAM10000
among its sources, but a perceptual-hash audit confirms HAM10000 images leaked
into it via reproduced literature figures, so image-level overlap is
established (Appendix~\ref{app:contamination}).
...
backbone swapped to BiomedCLIP (PMC-15M scientific-article figures,
with no documented raw HAM10000/ISIC archive ingestion). Test $\auroc =
