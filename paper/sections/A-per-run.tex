...
This appendix summarises the nine primary-tier runs and the subsequent
ten-run seed sweep. Every run has a self-contained report at
\texttt{docs/experiments/EXP-NNN-...md} in the source repository, and
every run's \texttt{manifest.json}, \texttt{embeddings/}, \texttt{metrics.json},
\texttt{baseline\_metrics.json}, \texttt{config.yaml}, \texttt{model\_card.md},
and \texttt{logs/} subtree are mirrored under \runarchive\ at
\texttt{<run\_id>/}.
...
\texttt{ham10000-hf-dinov2-exp004-v1}. Train stable pairs rotate between
...
CI[mean] $[0.3181, 0.3391]$). The seed-to-seed std ratio
